...
public final class TestLuminanceGradientParser {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
            ...
            final LuminanceGradientParser parser = new LuminanceGradientParser();
            final Iterator iterator = list.iterator();
            while (iterator.hasNext()) {
                final Location location = (Location)iterator.next();
                System.out.println("adding location " + location);
                region.add(location);
                final boolean error = parser.findError(region, location, 20);
                parser.add(region, location);
            }
		}
	}
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                while (tokens.hasMoreTokens()) {
                    ...
                    final double red = Double.parseDouble(components.nextToken()); 
                    final double green = Double.parseDouble(components.nextToken()); 
                    final double blue = Double.parseDouble(components.nextToken()); 
